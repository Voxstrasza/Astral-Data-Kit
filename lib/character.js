...
class Character
{
    ...
    sheet(race, cls, level, gear = {}, auraList = [])
    {
        ...
        const sheet = {
            ...
            /* Flat health and mana off an enchant or an old gem, which is the only way
               either pool moves apart from stamina and intellect. Wrath's own enchants use the
               stats instead; these are the pre-3.0 ones, and dropping them would make a
               '+100 Health' bracer enchant read as nothing at all. */
            health: base.baseHealth + Character.healthFromStamina(stats.sta) + (worn.health || 0),
            ...
            /*
             * Flat, not a rating: spell penetration takes resistance off the target directly and
             * has no entry in gtCombatRatings. Confirmed on the game's own spell tab on
             * 2026-08-30, where it sits under spell hit - which is where the panel puts it.
             */
            spellPen: worn.spellPen || 0,
        ...
        if (base.baseMana)
        {
            sheet.mana = base.baseMana + Character.manaFromIntellect(stats.int) + (worn.mana || 0);
