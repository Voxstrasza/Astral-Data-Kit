...
const KINDS = {
    ...
    /*
     * A whole character, which is a different thing again from the gear above it.
     *
     * `armory` is one wearable piece; this is the person wearing them - race, class, level, the
     * talents, the title and everything in the slots. Its own folder for the same reason the gear
     * got one: the two are answers to different questions, and a list of characters that filled up
     * with individual gauntlets would be useless.
     *
     * There is no icon. A character is found by its name and the line under it, which the panel
     * writes as "<Name> Level 80 Night Elf Hunter" - a race and a class say more about which
     * character this is than any one icon could.
     */
    character: { folder: 'characters', name: 'armoryName', icon: '' }
};
