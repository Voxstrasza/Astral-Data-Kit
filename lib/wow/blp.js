 ...
 *
 * Three encodings appear in practice: a 256-color palette with optional alpha,
...
function decode(buffer)
{
    ...
    {
        decodeDXT(data, rgba, width, height, dxtFlavor(alphaDepth, alphaEncoding));
    }
...
function dxtFlavor(alphaDepth, alphaEncoding)
{
    if (alphaEncoding === 0)
    {
        return 1;
...
function decodeDXT(data, rgba, width, height, flavor)
{
    const blocksX = Math.ceil(width / 4);
    const blocksY = Math.ceil(height / 4);
    const blockBytes = flavor === 1 ? 8 : 16;
    ...
    for (let by = 0; by < blocksY; ++by)
    {
        for (let bx = 0; bx < blocksX; ++bx)
        {
            ...
            const colorOffset = flavor === 1 ? offset : offset + 8;
            ...
            // DXT1 uses the color ordering to signal a 1-bit alpha mode.
            const opaqueMode = flavor !== 1 || value0 > value1;
