 ...
 * 24-bit BGR, 32-bit BGRA or 8-bit grayscale. Color-mapped TGAs are refused by name rather than
...
function looksLikeTga(buffer)
{
    ...
    const knownType = [
        TYPE.COLOR_MAPPED, TYPE.TRUE_COLOR, TYPE.GRAYSCALE,
        TYPE.RLE_COLOR_MAPPED, TYPE.RLE_TRUE_COLOR, TYPE.RLE_GRAYSCALE
    ].includes(imageType);
...
function decode(buffer)
{
    ...
    const colorMapDepth = buffer.readUInt8(7);
    const width = buffer.readUInt16LE(12);
    const height = buffer.readUInt16LE(14);
    const depth = buffer.readUInt8(16);
    const descriptor = buffer.readUInt8(17);

    if (imageType === TYPE.COLOR_MAPPED || imageType === TYPE.RLE_COLOR_MAPPED)
    {
        throw new Error('That TGA is color-mapped, which this cannot read. Save it as 24- or 32-bit.');
    }

    if (![TYPE.TRUE_COLOR, TYPE.GRAYSCALE, TYPE.RLE_TRUE_COLOR, TYPE.RLE_GRAYSCALE].includes(imageType))
    {
    ...
    const compressed = imageType === TYPE.RLE_TRUE_COLOR || imageType === TYPE.RLE_GRAYSCALE;

    // The pixels start after the id field and the color map, even when the map is unused.
    let offset = HEADER_SIZE + idLength + (colorMapType === 1 ? colorMapLength * Math.ceil(colorMapDepth / 8) : 0);
