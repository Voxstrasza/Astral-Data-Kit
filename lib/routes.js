...
/*
 * The app's data API, shared by the Electron shell and the plain-Node server so both behave
 * identically. Each handler returns { status, type, body } and never touches transport details.
 */
...
{
    const { assets, settings, worldDb, itemDisplay, itemBudget, raids, saved, portraitCameras } = ctx;
    ...
    if (pathname === '/api/item/search')
    {
        ...
        try
        {
            const rows = await worldDb.searchItems(query);
    ...
    if (pathname === '/api/item/get')
    {
        ...
        try
        {
            ...
            return json({
                item: items.toEditor(row, itemDisplay.icon(row.displayid), itemDisplay.enchantName(row.socketBonus))
            });
        }
        catch (err)
        {
    ...
    if (pathname === '/api/item/loot')
    {
        ...
        const entry = Number(params.get('creature'));
        ...
        try
        {
             ...
             */
            const fromCreature = entry ? await worldDb.lootForCreature(entry, { also }) : [];
...
module.exports = { handle, MODEL_CONTENT, MODEL_SCRIPT };
