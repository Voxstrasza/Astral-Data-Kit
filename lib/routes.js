...
async function handle(ctx, pathname, params, requestBody)
{
    ...
    if (pathname === '/api/gem/search')
    {
        ...
        try
        {
            ...
            /*
             * What a socket will physically take, which is a looser question than what earns
             * its bonus.
             *
             * The game lets any gem into any socket and simply withholds the socket bonus when
             * the colors disagree - a Bold Cardinal Ruby goes into a yellow socket, it just does
             * not pay. So the color match is not a filter here at all; it is `socketBonusMet`,
             * which already asks the same question of what is actually socketed.
             *
             * Meta is the one real restriction: a meta gem fits nothing but a meta socket, and a
             * meta socket takes nothing but a meta gem. An unnamed socket filters nothing, so
             * the same endpoint can still list every gem there is.
             */
            const META = items.SOCKET_MASKS.meta;

            const fitsSocket = (gem) =>
            {
                if (!socket)
                {
                    return true;
    ...
    if (pathname === '/api/enchant/list')
    {
        ...
        try
        {
            ...
            for (const one of spells.enchantSpells())
            {
                ...
                /*
                 * Which line a slot reads, where the two names disagree.
                 *
                 * The enchantment row is normally the right one - a weapon says "Berserking"
                 * rather than "Enchant Weapon - Berserking", which is why it is preferred. Ten
                 * enchants in the client also grant a spell you can use, which is what an
                 * engineering tinker is, and two of those rows are named after the stat they
                 * happen to carry rather than after themselves: Nitro Boosts reads "+24 Critical
                 * Strike Rating" while Mind Amplification Dish beside it reads its own name.
                 * A tinker is a thing with a name, so the spell wins those two.
                 *
                 * Only the label. The stats come off `effects` in `enchantStats`, never off this
                 * line, so Nitro Boosts still gives its 24 crit rating whatever it is called.
                 */
                const tinker = (row.effects || []).some(
                    (effect) => effect.type === items.ENCHANT_USE_SPELL);

                const namedAfterItsStat = /^[+-]?\d/.test(row.name);
                ...
                results.push({
                    ...
                    text: tinker && namedAfterItsStat && one.name ? one.name : row.name,
                    effects: row.effects
                });
            }
