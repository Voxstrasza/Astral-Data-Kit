 ...
 *      within a few percent from item level 200 to 277 and across every armour slot.
 *
 ...
 * effect, no resilience — minimising the median absolute error against their real stat blocks.
 ...
 * reward for matching colours, not a stat the item is charged for.
 */
 ...
 *
 * A weapon is not budgeted differently from armour — it just spends part of its allowance on
 * damage. Measured against every clean epic weapon: a melee weapon carrying the full dps for its
 * item level lands on the same 1.89 as armour (one-hand 1.89, ranged 1.88, wand 1.89), and a
 * caster weapon, which carries about 60% of that damage, is paid for the difference in stats.
 *
 * The exchange rate is the striking part. Taking each caster weapon's stats above the armour
...
/** Reads a row of item_template's stat_type/stat_value pairs into a named stat block. */
function statsFromItem(row)
{
    const stats = {};

    for (let i = 1; i <= 10; i++)
    {
        const name = STAT_TYPES[row[`stat_type${i}`]];
        const value = Number(row[`stat_value${i}`]) || 0;
...
class ItemBudget
{
     ...
     *
     * Zero for armour, and zero for a melee weapon at full dps. A weapon carrying more damage than
    ...
    identify({ stats, quality = 4, inventoryType, sockets = 0, socketBonus = false, anyLevel = false, weapon = null })
    {
        ...
        for (const ilvl of candidates)
        {
             ...
             * armour budget alone puts it several tiers above where it belongs.
             */
...
module.exports = {
    ...
    WEAPON_DPS, QUALITY_MULTIPLIER, SOCKET_COST, SOCKET_BONUS_COST, costOf, statsFromItem
};
