...
const SPELL = (() =>
{
    const f = {
        ...
        /*
         * And which slots it may be applied to, which is the whole of how the Armory knows what
         * can be enchanted onto gloves.
         *
         * A bit per InventoryType, the same numbering `item_template.InventoryType` uses. Checked
         * against a real client: of the 601 spells with an enchant-item effect, 430 carry a mask,
         * and the Waist bit picks out two - which is right, because a belt in Wrath takes a buckle
         * rather than an enchant.
         */
        EquippedItemInventoryTypeMask: 70,
        Effect: [71, 72, 73],
...
/* SPELL_EFFECT_ENCHANT_ITEM, whose misc value is the SpellItemEnchantment row it applies. */
...
class Spells
{
    ...
    load()
    {
        ...
        const spells = spellTable.map((r) =>
        {
            ...
            return {
                ...
                itemInventoryMask: r.int(SPELL.EquippedItemInventoryTypeMask),
    ...
    static fillDescription(spell, byId)
    {
         ...
         */
        /*
         * `${...}` may be followed by a precision: `${$m3/-1000}.1` is the expression to one
         * decimal place, not the expression and then a full stop. Reading it as text is how a
         * half-second global cooldown came out as "0.5.1 sec".
         */
        text = text.replace(/\$\{([^{}]*)\}(?:\.(\d))?/g, (found, body, places) =>
        {
            ...
            const whole = `\${${filled}}${places === undefined ? '' : `.${places}`}`;
            ...
            try
            {
                ...
                return places === undefined ? round(out) : out.toFixed(Number(places));
            }
            catch
            {
                return whole;
    ...
    /*
     * `byId` is passed in rather than reached for. This is a static, so `this` is the class and has
     * no tables on it - a description that points at another spell needs the lookup handed over.
     */
    static toEditor(spell, byId)
    {
        ...
        return {
            ...
            description: Spells.fillDescription(spell, byId)
        };
    }
    ...
    /**
     * Every spell that enchants an item, with the enchantment it applies and the slots it fits.
     *
     * This is the table the client never writes down: which enchants a slot can take. Effect 53 is
     * SPELL_EFFECT_ENCHANT_ITEM and its misc value is the `SpellItemEnchantment` row, so a spell
     * with that effect is an enchant and the mask beside it says where it goes.
     *
     * The QA spells are dropped. Blizzard's own test rows sit in the client alongside the real
     * ones - "QAEnchant Gloves +5 Mining", "QAEnchant Cloak +3 Agility" - and they were never
     * obtainable, so listing them would pad every slot with enchants that do not exist.
     */
    enchantSpells()
    {
        if (this.enchantCache)
        {
            return this.enchantCache;
        }

        const out = [];

        for (const spell of this.load().spells)
        {
            const index = spell.effect.indexOf(ENCHANT_ITEM);

            if (index < 0 || !spell.miscValue[index])
            {
                continue;
            }

            if (/^QAEnchant/i.test(spell.name || ''))
            {
                continue;
            }

            /*
             * Two ways of saying where an enchant goes, and both are needed.
             *
             * 430 of them name inventory types - gloves, boots, a cloak. The other 171 name an
             * item class and a subclass mask instead, which is how every weapon enchant is
             * written: Mongoose does not go on "a main hand", it goes on a sword or an axe or a
             * dagger, whichever hand that happens to be in. Reading only the first kind loses
             * every weapon enchant in the game, which is how this was noticed.
             */
            out.push({
                spell: spell.id,
                name: spell.name,
                enchant: spell.miscValue[index],
                mask: spell.itemInventoryMask,
                itemClass: spell.itemClass,
                subclassMask: spell.itemSubclassMask
            });
        }

        this.enchantCache = out;
    ...
    search(query, limit = 40)
    {
        ...
        {
            return [Spells.toEditor(byId.get(Number(text)), byId)];
        }
        ...
        return [...starts, ...contains]
            .slice(0, limit)
            .map((spell) => Spells.toEditor(spell, byId));
    }
}
