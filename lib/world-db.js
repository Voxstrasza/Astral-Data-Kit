...
class WorldDb
{
    ...
    /**
     * Gems by name, or every gem when nothing is typed.
     *
     * Item class 3 and a gem property id, which together are what makes a row a gem you can put
     * in a socket rather than a jewelcrafting reagent that happens to be filed beside one. The
     * color is not filtered here: which colors fit a socket is the client's `GemProperties`, and
     * the caller has that table where the database does not.
     *
     * Ordered by item level rather than by quality, because gems come in tiers and the one being
     * looked for is nearly always the highest one whose name matches.
     */
    async searchGems(query, limit = 60)
    {
        if (!this.pool)
        {
            return [];
        ...
        const where = !text ? ''
            : numeric ? ' AND it.entry = ?'
                : ' AND it.name LIKE ?';

        const sql = `
            ${ITEM_SELECT}
            WHERE it.class = 3 AND it.GemProperties > 0${where}
            ORDER BY it.ItemLevel DESC, CHAR_LENGTH(it.name), it.name
            LIMIT ?
        `;

        const [rows] = await this.pool.query(
            sql, [...(text ? [numeric ? Number(text) : `%${text}%`] : []), Number(limit)]);
