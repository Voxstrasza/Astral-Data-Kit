...
const { membersFor, renameFor, isSuppressed } = require('./encounter-members');
...
class WorldDb
{
    ...
    async connect(config)
    {
        ...
        try
        {
            ...
            const [rows] = await this.pool.query('SELECT COUNT(*) AS n FROM creature_template');
            this.lastError = null;

            return { ok: true, creatures: rows[0].n };
        }
        catch (err)
        {
     ...
     */
    async searchItems(query, limit = 40)
    {
        if (!this.pool)
        {
            return [];
        ...
        const sql = `
            ${ITEM_SELECT}
            WHERE ${numeric ? 'it.entry = ?' : 'it.name LIKE ?'}
            ORDER BY ${numeric ? 'it.entry' : 'it.Quality DESC, CHAR_LENGTH(it.name), it.name'}
            LIMIT ?
        `;

        const [rows] = await this.pool.query(sql, [numeric ? Number(query) : `%${query}%`, Number(limit)]);
    ...
    async lootForCreature(entry, options = {})
    {
        ...
        const [rows] = await this.pool.query(
            `SELECT Entry, Item, Reference, Chance, QuestRequired, LootMode, GroupId, MinCount, MaxCount
             ...
             WHERE Entry IN (${unique.map(() => '?').join(',')})`, unique);
