...
class ClientAssets
{
    ...
    buildIndex(onProgress)
    {
        ...
        this.archives.forEach(({ relative, archive }, index) =>
        {
            ...
            for (const entry of archive.listFiles())
            {
                ...
                // Later archives win, which is exactly the patch behavior we want.
    ...
    /** Raw bytes of an archive entry, honoring patch order. */
    readEntry(fullPath)
    {
     ...
     */
    /**
     * Reads a file the index does not cover, by asking every archive in turn.
     *
     * Only Interface, Fonts and DBFilesClient are indexed (see INDEX_PREFIXES), which leaves the
     * character and creature models out — and those are exactly what a portrait camera has to be
     * read from. Searching later archives first matches the client's own override order, so a
     * model a patch replaced is the one that comes back.
     */
    readAnywhere(fullPath)
    {
        for (let i = this.archives.length - 1; i >= 0; --i)
        {
            const raw = this.archives[i].archive.readFile(fullPath);
